...
lathe is a coding-agent CLI (in the mold of codex / claude-code) built in Go on top of `agentscope-go`. It runs an LLM in a tool-use loop against the local workspace, with an interactive TUI and a non-interactive print mode. Module: `github.com/alanfokco/lathe`, Go 1.26.3 (see `go.mod`).

## Build, test, run

The module uses `agentscope-go` v2.0.6 as a **remote module dependency** (fetched from the Go module proxy). No local checkout or `replace` directive needed.
...
- **Toolkit**: without `--sandbox`, lathe uses agentscope's host builtins (Bash/Read/Write/Edit/Glob/Grep). With `--sandbox`, `internal/workspace` rebuilds those tools to route through a `workspace.Workspace` (Docker container mounting cwd, or E2B cloud). Sandbox setup failure fails loudly — **no silent fallback to host execution**. Skills and MCP tools discovered before the sandbox swap are re-added to the sandbox toolkit so they remain available.
...
- **Task subagent** (`internal/agent/task_tool.go`): the `Task` tool spawns a nested `Engine` with a **builtins-only toolkit (no `Task` → no recursion)**, sharing the parent's model + permission engine (non-interactive). In sandbox mode the subagent shares the workspace. `assembleAgent` rebuilds the TaskTool with the current chatModel so that `/model` propagation works correctly.

### Session persistence
`internal/session` writes JSONL transcripts to `~/.lathe/projects/<enc-cwd>/<id>.jsonl` (claude-code-style project dirs; first line is metadata, the rest are messages). `--resume <id>` loads by ID (scans project dirs); `--continue` loads the newest in cwd. Save failures are logged as warnings via logrus (no silent data loss).

### Auto-compact
`internal/agent/compress.go`: when conversation tokens exceed `0.8 × context_size`, older messages are summarized via structured output (`{task_overview, current_state, important_discoveries, next_steps, context_to_preserve}`), formatted into a template, and replace the old prefix; a `0.1 × ctx` tail is reserved (tool_call/result pairs kept together). `/compact` runs asynchronously as a `tea.Cmd` so the TUI remains responsive during compression. Emits a `Compacted` event.

### Config loading
`internal/config/config.go` `Load(Flags)`: resolution order is **flag > env > TOML > defaults**. The `ConfigPath` field in `Flags` overrides the default TOML path (`~/.lathe/config.toml`); setting it to `"none"` or `"/dev/null"` skips TOML loading entirely, which is used by tests to avoid pollution from the user's config.
...
## Recent fixes (code-review round, 2026-08)

- **Config test isolation**: `Flags.ConfigPath` field; tests set `"none"` to avoid reading user TOML.
- **Sandbox skill/MCP preservation**: `NewEngine` re-adds skills and MCP tool groups after swapping to the workspace toolkit.
- **SetModel → TaskTool rebuild**: `assembleAgent` rebuilds the TaskTool with the current `chatModel` so subagents use the new model.
- **Per-turn tok/s**: activity line uses `turnOut` (reset on each `submit`) instead of cumulative `cumOut`.
- **Async /compact**: `/compact` runs as a `tea.Cmd` returning a `compactDoneMsg`, with progress feedback — no TUI freeze.
- **Workspace glob shell-injection fix**: `BasePath()` is now quoted in the `find` command template.
- **Session save-failure logging**: `persistHook.OnLoopEnd` logs a warning on save error instead of silently discarding.
- **Viewport height expectation**: test updated for the 4-line bottom layout (M13b).
...
- Config resolution order is **flag > env > TOML > defaults** (`internal/config`). Wire new options through `config.Flags` / `config.Config` and `Load` rather than reading env ad hoc.
